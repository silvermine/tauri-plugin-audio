...
/// Returns `Ok` if the current status allows loading a new source.
pub fn can_load(status: PlaybackStatus) -> Result<()> {
   match status {
      PlaybackStatus::Idle | PlaybackStatus::Ended | PlaybackStatus::Error => Ok(()),
...
pub fn seek(state: &mut PlayerState, position: f64) -> Result<()> {
   ...
   state.current_time = position.max(0.0);
   Ok(())
}
