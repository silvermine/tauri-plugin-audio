   ...
   fn process_spectrum(&mut self, mut step: usize) {
      ...
      if step < self.channels {
         ...
         for b in 0..self.bands {
            ...
            self.channel_bands[start + b].output = phase / (prev_energy.max(energy) + NOISE_FLOOR);
         }
         return;
      }
      step -= self.channels;
      ...
      if step < SPLIT_MAIN_PREDICTION {
         ...
         for b in start_b..end_b {
            ...
            if b > 0 {
               ...
               let down_input = self
                  .get_fractional_input_float(max_channel, map_point.input_bin - bin_time_factor);
               ...
               if b >= long_vertical_step {
                  ...
                  let long_down_output =
                     self.channel_bands[max_channel * self.bands + b - long_vertical_step].output;
            ...
            if b + 1 < self.bands {
               ...
               if b + long_vertical_step < self.bands {
                  let long_up_prediction =
                     self.channel_predictions[max_channel * self.bands + b + long_vertical_step];
                  let long_up_map_point = self.output_map[b + long_vertical_step];
                  let long_down_input = self.get_fractional_input_float(
                     max_channel,
                     long_up_map_point.input_bin - long_vertical_step as f32 * bin_time_factor,
                  );
                  let long_vertical_twist = long_up_prediction.input.mul_conj_rhs(long_down_input);
                  let long_up_output =
                     self.channel_bands[max_channel * self.bands + b + long_vertical_step].output;
